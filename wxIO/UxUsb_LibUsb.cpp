...
int UxUsb_LibUsb::DeviceOpen(unsigned short vid, unsigned short pid)
{
   ...
   // rescan to find changes
   _V_ ::fprintf(stderr, "UxUsb_LibUsb::DeviceOpen - usb_find_busses\n");
   usb_find_busses();
   usb_find_devices();
   ...
   for (bus = m_busses; bus; bus = bus->next) {
      ...
      //_V_ ::fprintf(stderr, "UxUsb_LibUsb::DeviceOpen - %s \n", dev->filename);
      for (dev = bus->devices; dev; dev = dev->next) {
         // check if we search for that device
         if ( (dev->descriptor.idVendor==vid) && (dev->descriptor.idProduct== pid) ) {
            // Open the device, claim the interface and do your processing
            _V_ ::fprintf(stderr, "UxUsb_LibUsb::DeviceOpen Matches PID/VID\n");
            m_USBDevice = dev;
            ...
            if (m_DevHandle) {
               ...
               if (rVal != 0) {
                   ...
                   // This will cause the device to dissapear as the driver has been detatched
...
int UxUsb_LibUsb::DeviceResetAndClose()
{
   int err = USB_OK;
	...
	if (m_devOK) {
      _V_ ::fprintf(stderr, "UxUsb_LibUsb::DeviceResetAndClose() - device reset and close.\n");
	   err= usb_release_interface(m_DevHandle, 0);
	   err= usb_reset(m_DevHandle);    // disconect/reconnect device

      //Need to Reattach the FTDI Driver here but not supported in libusb 0.1
