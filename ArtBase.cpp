...
{
   _V_ ::fprintf(stderr, "ArtBase::EnumerateArtCameras\n");
   ...
   if (m_camCcdDrv) {
      ...
      _V_ ::fprintf(stderr, "ArtBase::Device %i\n",i);
         if ( m_camCcdDrv->DeviceIsCamera(i) ) {
            _V_ ::fprintf(stderr, "ArtBase::DeviceIsCamera Found\n");
   ...
   {
      _V_ ::fprintf(stderr, "ArtBase::m_camCcdDrv not found!\n");
   }
...
bool ArtBase::ControlDevice(long cfIndex)
{
   ...
   // This line cause the device to be delete from /dev although connection works ok.
