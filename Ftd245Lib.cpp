   ...
   _V_ ::fprintf(stderr, "Ftd245Lib::ShutConnection() - device closed.\n");
	if (m_deviceConnected) {
      ...
      //m_pDriver->DeviceClose();
		m_deviceConnected = false;
      _V_ ::fprintf(stderr, "Ftd245Lib::DeviceResetAndClose() - device reset and closed.\n");
	}
...
bool Ftd245Lib::InitBoard(int deviceIndex)
{
   ...
   err=ConnectDevice(deviceIndex); // This causes a connection to the usb device but removes it from /dev/
